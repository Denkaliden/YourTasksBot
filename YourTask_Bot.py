...
def send_welcome(message):
    bot.reply_to(message, "*/add ваш_текст* - Добавляет задачу в список\n\n*/remove номер_задачи* - убирает задачу по "
                          "номеру из списка\n\n*/list* - Показывает все ваши задачи и их номера\n\n*/clear* - очищает "
                          "весь список разом\n\n*/edit номер_задачи новый_текст* - Позволяет изменить текст одной из "
                          "задач\n\n*/set_timer номер_задачи время_в_минутах* - установка таймера на задачу, "
                          "чтобы бот напомнил вам о ней через время")
